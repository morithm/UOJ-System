...
if ($contest != null) {
	genMoreContestInfo($contest, $myUser);
...
if ($contest != null) {
	if (!hasContestPermission($myUser, $contest)) {
		if ($contest['cur_progress'] <= CONTEST_NOT_STARTED) {
			...
			becomeMsgPage("<p>很遗憾，比赛已结束，您无法查看题目。</p>");
		} else {
			becomeMsgPage("<p>很遗憾，您尚未报名，无法查看题目。</p>");
...
function handleUpload($zip_file_name, $content, $tot_size)
{
	...
	if ($is_in_contest) {
		DB::query("insert into submissions (problem_id, contest_id, submit_time, submitter, content, language, tot_size, status, result, is_hidden) values (${problem['id']}, ${contest['id']}, now(), '${myUser['username']}', '$esc_content', '$esc_language', $tot_size, '${result['status']}', '$result_json', {$problem['is_hidden']})");
	} else {
...
<?php if ($contest) : ?>
	<div class="page-header row">
		...
		<h1 class="col-md-7"><?= $problem_letter ?>. <?= $problem['title'] ?></h1>
...
<div class="tab-content">
	<div class="tab-pane active" id="tab-statement">
		<div class="row">
			<article class="col-sm-9 col-xs-12 top-buffer-md"><?= $problem_content['statement'] ?></article>
			<session class="side col-sm-3">
				<div class="card my-2">
					<table class="table mb-0">
						<tr>
							<td>时间限制</td>
							<td><?= $time_limit != null ? "$time_limit s" : "N/A" ?></td>
						</tr>
						<tr>
							<td>空间限制</td>
							<td><?= $memory_limit != null ? "$memory_limit MB" : "N/A" ?></td>
						</tr>
						<tr>
							<td>难度</td>
							<td><?= UOJLocale::get('problems::level', $problem["level"]) ?></td>
						</tr>
						<tr>
							<td>
								<button id="problem-tags-btn" class="btn btn-link collapsed text-nowrap" data-toggle="collapse" data-target="#problem-tags" aria-expanded="false" aria-controls="problem-tags">
									<span class="if-collapsed">显示标签</span>
									<span class="if-not-collapsed">隐藏标签</span>
								</button>
								<div id="problem-tags" class="collapse p-2">
									<?php
...
<script>
	$(function() {
		let $copy_btn = $('<span class="fa-copy">Copy</span>');
		$('article pre').css("position", "relative");
		$('article pre').prepend($copy_btn);

		$('.fa-copy').tooltip({
			trigger: 'click'
		});

		let setTooltip = function(btn, message) {
			$(btn).tooltip('hide')
		...
		let clipboard = new ClipboardJS('.fa-copy', {
			text: function(trigger) {
				return $(trigger).nextAll("code").text();
			}
		});
