...
function export($file, $result)
{
	$out = fopen($file, 'w');
	try {
		while ($row = DB::fetch($result)) {
			fputcsv($out, mb_convert_encoding($row, "CP936", "UTF-8"));
		}
	} catch (Exception $e) {
	}
	fclose($out);
...
$problem_import->runAtServer();

$problem_export = new UOJForm('problem_export');
$problem_export->addInput(
	'begin_id',
	'text',
	'起始题号',
	'',
	function ($begin_id) {
		if (!ValidateUInt($begin_id)) {
			return '起始题号不合法';
		}
		if (!queryProblemBrief($begin_id)) {
			return '起始题号不存在';
		}
		return '';
	},
...
$problem_export->addInput(
	...
	'',
	function ($end_id) {
		if (!ValidateUInt($end_id)) {
			return '结束题号不合法';
		}
		if (!queryProblemBrief($end_id)) {
			return '结束题号不存在';
		}
		return '';
	},
...
$problem_export->handle = function () {
	$begin_id = $_POST['begin_id'];
	$end_id = $_POST['end_id'];
	if ($begin_id > $end_id)
		$begin_id ^= $end_id ^= $begin_id ^= $end_id;
	$token = uojRandString(16);
	$work_dir = '/tmp/' . $token . '/';
	if (is_dir($work_dir) === false) {
		mkdir($work_dir);
	}
	$result = DB::query("select * from problems where id between {$begin_id} and {$end_id}");
	export($work_dir . 'problems.csv', $result);
	$result = DB::query("select * from problems_contents where id between {$begin_id} and {$end_id}");
	export($work_dir . 'problems_contents.csv', $result);
	for ($id = $begin_id; $id <= $end_id; $id++) {
		$file_name = "/var/uoj_data/$id.zip";
		copy($file_name, $work_dir . $id . '.zip');
	}

	unlink('/var/www/uoj/files/file.zip');
	// Initialize archive object
	$zip = new ZipArchive();
	$zip->open('/var/www/uoj/files/file.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE);

	// Create recursive directory iterator
	/** @var SplFileInfo[] $files */
	$files = new RecursiveIteratorIterator(
		new RecursiveDirectoryIterator($work_dir),
	...
	rmdir($work_dir);
	redirectTo('/files/file.zip');
	return '';
};
$problem_export->runAtServer();
